...
-- Hallazgo real de uso 2026-08-10: `POST /admin/metadata-keys` sólo sellaba
-- la clave privada nueva para el propio admin que la creaba — ningún otro
-- usuario, nunca, tuvo acceso a ninguna metadata key compartida en la
-- práctica. `POST /admin/metadata-keys/{id}/members` (nuevo) permite
-- agregar un miembro a una key ya activa sin rotarla — necesita su propio
-- evento de auditoría.
