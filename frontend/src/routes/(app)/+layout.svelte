...
<script lang="ts">
	...
	const enlaces = $derived([
		...
		{ href: '/health', label: $t.appShell.salud, icono: ICONOS.salud },
		...
		...($externalSharePolicy?.enabled
			...
			: []),
		// F-46: modo escritorio es de 1 solo usuario (el backend ya rechaza
		// un segundo registro y no expone /admin/users) — el panel entero de
		// administración de equipo no tiene sentido ahí, mostrarlo era
		// navegar a secciones que en el mejor caso no hacen nada.
		...($esAdmin && !enModoEscritorio() ? [{ href: '/admin', label: $t.appShell.administracion, icono: ICONOS.admin }] : [])
	]);
	...
	// Atajo de teclado estándar de escritorio (Ctrl+B / Cmd+B) para colapsar/expandir el Panel 1
	$effect(() => {
...
{#if !verificando}
	<div class="shell" class:colapsado>
		<nav>
			...
			{#if $carpetasSidebar && !colapsado}
				<div class="carpetas-sidebar">
					<FolderTree
						nodos={$carpetasSidebar.nodos}
						cargando={$carpetasSidebar.cargando}
						filtroActivo={$carpetasSidebar.filtroActivo}
						onCrear={$carpetasSidebar.onCrear}
						onMover={$carpetasSidebar.onMover}
						onFiltrar={$carpetasSidebar.onFiltrar}
						onCompartir={$carpetasSidebar.onCompartir}
					/>
				</div>
			{/if}
			<div class="pie">
...
<style>
	...
	nav {
		...
		position: sticky;
		/* Debajo de la barra de título propia (0 en la web). */
		top: var(--titlebar-h);
	...
	/* Antes `ul` tenía `flex: 1` para empujar `.pie` al fondo del nav
	   (sticky, 100vh) — con el árbol de carpetas de por medio, forzar a `ul`
	   a crecer empujaría tanto las carpetas como el pie fuera del viewport.
	   `margin-top: auto` en `.pie` logra lo mismo (pegarlo abajo) sin
	   importar si `.carpetas-sidebar` está presente o no. */
	.carpetas-sidebar {
		flex: 1;
		min-height: 0;
		display: flex;
	...
	main {
		...
		overflow-x: auto;
		/* Sin esto, el track `1fr` del grid no se achica por debajo del ancho
		   intrínseco del contenido (default `min-width: auto` de todo ítem
		   de grid/flex) — el `overflow-x: auto` de arriba nunca llega a
		   activarse porque el grid entero crece en cambio. Mismo bug
		   encontrado en `vault/+page.svelte`, acá en la capa que envuelve
		   TODAS las páginas. */
		min-width: 0;
	}
</style>
