...
function parsearHostPuerto(uri) {
...
function comandoDeConexion(resourceTypeSlug, usuario, uri) {
	...
	const puertoNoDefault = puerto !== void 0 && puerto !== puertoDefault ? puerto : void 0;
	const arroba = usuario ? `${usuario}@` : "";
	switch (resourceTypeSlug) {
		case "ssh": return `ssh ${arroba}${host}${puertoNoDefault ? ` -p ${puertoNoDefault}` : ""}`;
		case "telnet": return `telnet ${host}${puertoNoDefault ? ` ${puertoNoDefault}` : ""}`;
		case "ftp": return `ftp://${arroba}${host}${puertoNoDefault ? `:${puertoNoDefault}` : ""}`;
		case "vnc": return `vnc://${arroba}${host}${puertoNoDefault ? `:${puertoNoDefault}` : ""}`;
		default: return null;
	}
}
...
function urlAbrible(resourceTypeSlug, uri) {
	if (!uri || resourceTypeSlug in PUERTOS_DEFAULT) return null;
...
function hostnameParaFavicon(resourceTypeSlug, uri) {
	if (!uri || resourceTypeSlug in PUERTOS_DEFAULT) return null;
	try {
...
//#endregion
//#region ../frontend/src/lib/crypto/passwordGenerator.ts
var UPPER = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
var UPPER_SIN_AMBIGUOS = "ABCDEFGHJKLMNPQRSTUVWXYZ";
var LOWER = "abcdefghijklmnopqrstuvwxyz";
var LOWER_SIN_AMBIGUOS = "abcdefghjkmnpqrstuvwxyz";
var DIGITS = "0123456789";
var DIGITS_SIN_AMBIGUOS = "23456789";
var SYMBOLS = "!@#$%^&*()-_=+[]{}";
function indiceSinSesgo(rango) {
	const max = Math.floor(4294967296 / rango) * rango;
...
function generarPassword(longitud, reglas) {
	const grupos = [];
	...
	while (resultado.length < longitud) resultado.push(alfabeto[indiceSinSesgo(alfabeto.length)]);
...
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/frequency_lists.js
var require_frequency_lists = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
...
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/adjacency_graphs.js
var require_adjacency_graphs = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
		qwerty: {
			"!": [
				"`~",
...
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/scoring.js
var require_scoring = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var BRUTEFORCE_CARDINALITY;
	var MIN_GUESSES_BEFORE_GROWING_SEQUENCE;
	var MIN_SUBMATCH_GUESSES_MULTI_CHAR;
	var MIN_SUBMATCH_GUESSES_SINGLE_CHAR;
	var adjacency_graphs = require_adjacency_graphs();
	var calc_average_degree = function(graph) {
		var average = 0, k, key, n, neighbors;
		for (key in graph) {
			neighbors = graph[key];
			average += (function() {
				var len, o, results = [];
	...
	scoring = {
		...
		factorial: function(n) {
			...
			if (n < 2) return 1;
			f = 1;
			for (i = o = 2, ref = n; 2 <= ref ? o <= ref : o >= ref; i = 2 <= ref ? ++o : --o) f *= i;
			return f;
		},
		most_guessable_match_sequence: function(password, matches, _exclude_additive) {
			...
			matches_by_j = (function() {
				var o, ref, results = [];
				for (o = 0, ref = n; 0 <= ref ? o < ref : o > ref; 0 <= ref ? ++o : --o) results.push([]);
				return results;
			})();
			...
			optimal = {
				m: (function() {
					var ref, results = [], u;
					for (u = 0, ref = n; 0 <= ref ? u < ref : u > ref; 0 <= ref ? ++u : --u) results.push({});
					return results;
				})(),
				pi: (function() {
					var ref, results = [], u;
			...
			update = (function(_this) {
				return function(m, l) {
					var competing_g, competing_l, g, k = m.j, pi = _this.estimate_guesses(m, password), ref;
					if (l > 1) pi *= optimal.pi[m.i - 1][l - 1];
					g = _this.factorial(l) * pi;
					if (!_exclude_additive) g += Math.pow(MIN_GUESSES_BEFORE_GROWING_SEQUENCE, l - 1);
					ref = optimal.g[k];
				...
				return function(k) {
					...
					for (i = u = 1, ref = k; 1 <= ref ? u <= ref : u >= ref; i = 1 <= ref ? ++u : --u) {
						...
						results.push((function() {
							var ref1 = optimal.m[i - 1], results1 = [];
							for (l in ref1) {
								last_m = ref1[l];
								l = parseInt(l);
			...
			for (k = u = 0, ref = n; 0 <= ref ? u < ref : u > ref; k = 0 <= ref ? ++u : --u) {
				...
				for (w = 0, len2 = ref1.length; w < len2; w++) {
					m = ref1[w];
					if (m.i > 0) for (l in optimal.m[m.i - 1]) {
						l = parseInt(l);
						update(m, l + 1);
					}
					else update(m, 1);
			...
			else guesses = optimal.g[n - 1][optimal_l];
			return {
				password,
				guesses,
		...
		estimate_guesses: function(match, password) {
			...
			min_guesses = 1;
			if (match.token.length < password.length) min_guesses = match.token.length === 1 ? MIN_SUBMATCH_GUESSES_SINGLE_CHAR : MIN_SUBMATCH_GUESSES_MULTI_CHAR;
		...
		bruteforce_guesses: function(match) {
			var guesses = Math.pow(BRUTEFORCE_CARDINALITY, match.token.length), min_guesses;
		...
		sequence_guesses: function(match) {
			var base_guesses, first_chr = match.token.charAt(0);
			if (first_chr === "a" || first_chr === "A" || first_chr === "z" || first_chr === "Z" || first_chr === "0" || first_chr === "1" || first_chr === "9") base_guesses = 4;
			else if (first_chr.match(/\d/)) base_guesses = 10;
			else base_guesses = 26;
			if (!match.ascending) base_guesses *= 2;
		...
		regex_guesses: function(match) {
			...
			if (match.regex_name in char_class_bases) return Math.pow(char_class_bases[match.regex_name], match.token.length);
		...
		date_guesses: function(match) {
			var guesses = Math.max(Math.abs(match.year - this.REFERENCE_YEAR), this.MIN_YEAR_SPACE) * 365;
		...
		KEYBOARD_STARTING_POSITIONS: (function() {
			var ref = adjacency_graphs.qwerty, results = [];
			for (k in ref) {
				ref[k];
				results.push(k);
			}
		...
		KEYPAD_STARTING_POSITIONS: (function() {
			var ref = adjacency_graphs.keypad, results = [];
			for (k in ref) {
				ref[k];
				results.push(k);
			}
		...
		spatial_guesses: function(match) {
			...
			for (i = o = 2, ref1 = L; 2 <= ref1 ? o <= ref1 : o >= ref1; i = 2 <= ref1 ? ++o : --o) {
				...
				for (j = q = 1, ref2 = possible_turns; 1 <= ref2 ? q <= ref2 : q >= ref2; j = 1 <= ref2 ? ++q : --q) guesses += this.nCk(i - 1, j - 1) * s * Math.pow(d, j);
			}
			if (match.shifted_count) {
				...
				if (S === 0 || U === 0) guesses *= 2;
					...
					for (i = u = 1, ref3 = Math.min(S, U); 1 <= ref3 ? u <= ref3 : u >= ref3; i = 1 <= ref3 ? ++u : --u) shifted_variations += this.nCk(S + U, i);
		...
		uppercase_variations: function(match) {
			var L, U, chr, i, len, o, q, ref, ref1, regex, variations, word = match.token;
			...
			U = (function() {
				var len1, q, ref1 = word.split(""), results = [];
				for (q = 0, len1 = ref1.length; q < len1; q++) {
					chr = ref1[q];
					if (chr.match(/[A-Z]/)) results.push(chr);
				}
				return results;
			})().length;
			L = (function() {
			...
			variations = 0;
			for (i = q = 1, ref1 = Math.min(U, L); 1 <= ref1 ? q <= ref1 : q >= ref1; i = 1 <= ref1 ? ++q : --q) variations += this.nCk(U + L, i);
			return variations;
		},
			...
			for (subbed in ref) {
				...
				S = (function() {
					...
					for (o = 0, len = chrs.length; o < len; o++) {
						chr = chrs[o];
						if (chr === subbed) results.push(chr);
					}
					return results;
				})().length;
				U = (function() {
					...
					for (o = 0, len = chrs.length; o < len; o++) {
						chr = chrs[o];
						if (chr === unsubbed) results.push(chr);
					}
					return results;
				})().length;
				if (S === 0 || U === 0) variations *= 2;
					...
					p = Math.min(U, S);
					possibilities = 0;
					for (i = o = 1, ref1 = p; 1 <= ref1 ? o <= ref1 : o >= ref1; i = 1 <= ref1 ? ++o : --o) possibilities += this.nCk(U + S, i);
...
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/matching.js
var require_matching = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var DATE_MAX_YEAR;
	var DATE_MIN_YEAR;
	var DATE_SPLITS;
	var GRAPHS;
	...
	var frequency_lists = require_frequency_lists();
	var lst;
	var matching;
	var name;
	var scoring;
	...
	matching = {
		...
		translate: function(string, chr_map) {
			var chr;
			return (function() {
				var len1, o, ref = string.split(""), results = [];
		...
		dictionary_match: function(password, _ranked_dictionaries) {
			...
			for (dictionary_name in _ranked_dictionaries) {
				...
				for (i = o = 0, ref = len; 0 <= ref ? o < ref : o > ref; i = 0 <= ref ? ++o : --o) for (j = p = ref1 = i, ref2 = len; ref1 <= ref2 ? p < ref2 : p > ref2; j = ref1 <= ref2 ? ++p : --p) if (password_lower.slice(i, +j + 1 || 9e9) in ranked_dict) {
		...
		relevant_l33t_subtable: function(password, table) {
			var chr, len1, letter, o, password_chars = {}, ref = password.split(""), relevant_subs, sub, subs, subtable;
			for (o = 0, len1 = ref.length; o < len1; o++) {
				chr = ref[o];
			...
			for (letter in table) {
				...
				relevant_subs = (function() {
					...
					for (p = 0, len2 = subs.length; p < len2; p++) {
						sub = subs[p];
						if (sub in password_chars) results.push(sub);
					}
					return results;
				})();
				if (relevant_subs.length > 0) subtable[letter] = relevant_subs;
			}
			return subtable;
		},
		enumerate_l33t_subs: function(table) {
			var chr, dedup, helper, k, keys = (function() {
				var results = [];
				for (k in table) results.push(k);
				return results;
			})(), l33t_chr, len1, len2, o, p, ref, sub, sub_dict, sub_dicts, subs = [[]];
			dedup = function(subs) {
			...
			helper = function(keys) {
				...
				for (o = 0, len1 = ref.length; o < len1; o++) {
					...
					for (p = 0, len2 = subs.length; p < len2; p++) {
						sub = subs[p];
						dup_l33t_index = -1;
						for (i = q = 0, ref1 = sub.length; 0 <= ref1 ? q < ref1 : q > ref1; i = 0 <= ref1 ? ++q : --q) if (sub[i][0] === l33t_chr) {
							dup_l33t_index = i;
							break;
						}
						if (dup_l33t_index === -1) {
			...
			for (o = 0, len1 = ref.length; o < len1; o++) {
				...
				for (p = 0, len2 = ref1.length; p < len2; p++) {
					...
					for (subbed_chr in sub) {
						chr = sub[subbed_chr];
						if (token.indexOf(subbed_chr) !== -1) match_sub[subbed_chr] = chr;
					}
					match.l33t = true;
					match.token = token;
					match.sub = match_sub;
		...
		spatial_match_helper: function(password, graph, graph_name) {
			...
			while (i < password.length - 1) {
				...
				last_direction = null;
				turns = 0;
				if ((graph_name === "qwerty" || graph_name === "dvorak") && this.SHIFTED_RX.exec(password.charAt(i))) shifted_count = 1;
				else shifted_count = 0;
				while (true) {
					...
					if (j < password.length) {
						...
						for (o = 0, len1 = adjacents.length; o < len1; o++) {
							...
							if (adj && adj.indexOf(cur_char) !== -1) {
								...
								if (adj.indexOf(cur_char) === 1) shifted_count += 1;
		...
		sequence_match: function(password) {
			...
			for (k = o = 1, ref = password.length; 1 <= ref ? o < ref : o > ref; k = 1 <= ref ? ++o : --o) {
				...
				if (last_delta == null) last_delta = delta;
		...
		date_match: function(password) {
			var best_candidate, candidate, candidates, distance, dmy, i, j, k, l, len1, len2, matches = [], maybe_date_no_separator = /^\d{4,8}$/, maybe_date_with_separator = /^(\d{1,4})([\s\/\\_.-])(\d{1,2})\2(\d{1,4})$/, metric, min_distance, o, p, q, r, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, ref9, rx_match, s, t, token;
			for (i = o = 0, ref = password.length - 4; 0 <= ref ? o <= ref : o >= ref; i = 0 <= ref ? ++o : --o) for (j = p = ref1 = i + 3, ref2 = i + 7; ref1 <= ref2 ? p <= ref2 : p >= ref2; j = ref1 <= ref2 ? ++p : --p) {
				...
				for (r = 0, len2 = ref5.length; r < len2; r++) {
					...
					if (distance < min_distance) ref6 = [candidate, distance], best_candidate = ref6[0], min_distance = ref6[1];
				}
				matches.push({
					pattern: "date",
					token,
			...
			for (i = s = 0, ref7 = password.length - 6; 0 <= ref7 ? s <= ref7 : s >= ref7; i = 0 <= ref7 ? ++s : --s) for (j = t = ref8 = i + 5, ref9 = i + 9; ref8 <= ref9 ? t <= ref9 : t >= ref9; j = ref8 <= ref9 ? ++t : --t) {
				if (j >= password.length) break;
				token = password.slice(i, +j + 1 || 9e9);
			...
			return this.sorted(matches.filter(function(match) {
				var is_submatch = false, len3, other_match, u;
		...
		map_ints_to_dm: function(ints) {
			var d, len1, m, o, ref = [ints, ints.slice().reverse()], ref1;
			for (o = 0, len1 = ref.length; o < len1; o++) {
				ref1 = ref[o], d = ref1[0], m = ref1[1];
				if (1 <= d && d <= 31 && 1 <= m && m <= 12) return {
					day: d,
					month: m
				};
			}
		...
		two_to_four_digit_year: function(year) {
			...
			else if (year > 50) return year + 1900;
			else return year + 2e3;
		}
	};
	module.exports = matching;
}));
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/time_estimates.js
var require_time_estimates = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = {
		estimate_attack_times: function(guesses) {
			var crack_times_display, crack_times_seconds = {
		...
		display_time: function(seconds) {
			var base, century, day, display_num, display_str, hour, minute = 60, month, ref, year;
			hour = minute * 60;
			day = hour * 24;
			month = day * 31;
			year = month * 12;
...
//#endregion
//#region ../frontend/node_modules/.pnpm/zxcvbn@4.4.2/node_modules/zxcvbn/lib/feedback.js
var require_feedback = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	var feedback;
	...
	feedback = {
		...
		get_match_feedback: function(match, is_sole_match) {
			...
			switch (match.pattern) {
				case "dictionary": return this.get_dictionary_match_feedback(match, is_sole_match);
				case "spatial":
				...
				case "regex":
					if (match.regex_name === "recent_year") return {
		...
		get_dictionary_match_feedback: function(match, is_sole_match) {
			var ref, result, suggestions, warning = match.dictionary_name === "passwords" ? is_sole_match && !match.l33t && !match.reversed ? match.rank <= 10 ? "This is a top-10 common password" : match.rank <= 100 ? "This is a top-100 common password" : "This is a very common password" : match.guesses_log10 <= 4 ? "This is similar to a commonly used password" : void 0 : match.dictionary_name === "english_wikipedia" ? is_sole_match ? "A word by itself is easy to guess" : void 0 : (ref = match.dictionary_name) === "surnames" || ref === "male_names" || ref === "female_names" ? is_sole_match ? "Names and surnames by themselves are easy to guess" : "Common names and surnames are easy to guess" : "", word;
			suggestions = [];
			word = match.token;
			if (word.match(scoring.START_UPPER)) suggestions.push("Capitalization doesn't help very much");
			else if (word.match(scoring.ALL_UPPER) && word.toLowerCase() !== word) suggestions.push("All-uppercase is almost as easy to guess as all-lowercase");
...
//#endregion
//#region ../frontend/src/lib/crypto/passwordStrength.ts
var import_main = /* @__PURE__ */ __toESM((/* @__PURE__ */ __commonJSMin(((exports, module) => {
	var feedback;
	var matching = require_matching();
	var scoring = require_scoring();
	var time;
	var time_estimates = require_time_estimates();
	var zxcvbn;
	...
	zxcvbn = function(password, user_inputs) {
		...
		for (i = 0, len = user_inputs.length; i < len; i++) {
			arg = user_inputs[i];
			if ((ref = typeof arg) === "string" || ref === "number" || ref === "boolean") sanitized_inputs.push(arg.toString().toLowerCase());
		}
...
function evaluarFortaleza(password) {
	...
	const resultado = (0, import_main.default)(password);
	const feedback = [resultado.feedback.warning, ...resultado.feedback.suggestions].filter((s) => !!s);
	return {
...
var botonNuevo = document.getElementById("boton-nuevo");
var mensajeNoSoportado = document.getElementById("mensaje-no-soportado");
var botonVolver = document.getElementById("boton-volver");
var botonVolverLista = document.getElementById("boton-volver-lista");
var detalleTitulo = document.getElementById("detalle-titulo");
var detalleUsuario = document.getElementById("detalle-usuario");
var botonCopiarUsuario = document.getElementById("boton-copiar-usuario");
var detallePassword = document.getElementById("detalle-password");
var detallePasswordFortaleza = document.getElementById("detalle-password-fortaleza");
var botonRevelarPassword = document.getElementById("boton-revelar-password");
var botonCopiarPassword = document.getElementById("boton-copiar-password");
var detalleSitioBloque = document.getElementById("detalle-sitio-bloque");
var detalleSitioLink = document.getElementById("detalle-sitio-link");
var detalleComandoBloque = document.getElementById("detalle-comando-bloque");
var detalleComando = document.getElementById("detalle-comando");
var botonCopiarComando = document.getElementById("boton-copiar-comando");
var detalleTotpBloque = document.getElementById("detalle-totp-bloque");
var detalleTotp = document.getElementById("detalle-totp");
var botonCopiarTotp = document.getElementById("boton-copiar-totp");
var detalleNotaBloque = document.getElementById("detalle-nota-bloque");
var detalleNota = document.getElementById("detalle-nota");
var detalleAbrirWeb = document.getElementById("detalle-abrir-web");
var detalleCreado = document.getElementById("detalle-creado");
var detalleModificado = document.getElementById("detalle-modificado");
var FORMATO_FECHA = new Intl.DateTimeFormat("es", {
	day: "numeric",
	month: "short",
	year: "numeric"
});
var botonEditarItem = document.getElementById("boton-editar-item");
var formRecurso = document.getElementById("form-recurso");
var formTitulo = document.getElementById("form-titulo");
var formTipo = document.getElementById("form-tipo");
var formNombre = document.getElementById("form-nombre");
var formUsuario = document.getElementById("form-usuario");
var formUri = document.getElementById("form-uri");
var formPassword = document.getElementById("form-password");
var botonGenerarPassword = document.getElementById("boton-generar-password");
var botonMostrarFormPassword = document.getElementById("boton-mostrar-form-password");
var formTotp = document.getElementById("form-totp");
var formNotas = document.getElementById("form-notas");
var errorForm = document.getElementById("error-form");
var botonGuardarForm = document.getElementById("boton-guardar-form");
var botonCancelarForm = document.getElementById("boton-cancelar-form");
var botonVolverCuenta = document.getElementById("boton-volver-cuenta");
var cuentaEmail = document.getElementById("cuenta-email");
var cuentaServidor = document.getElementById("cuenta-servidor");
var cuentaAbrirWeb = document.getElementById("cuenta-abrir-web");
var ICONO_POR_TIPO = {
	"login-password": "🔑",
	ssh: "💻",
	ftp: "📁",
...
function renderizarLista(filtro) {
	...
	let filtrados = q ? itemsVault.filter((i) => i.nombre.toLowerCase().includes(q) || i.usuario.toLowerCase().includes(q) || i.uri.toLowerCase().includes(q)) : itemsVault;
...
var itemDetalleActual = null;
var secretoRevelado = null;
var servidorActual = "";
...
async function obtenerSecretoRevelado() {
	if (secretoRevelado) return secretoRevelado;
	if (!itemDetalleActual) throw new Error("no hay ningún ítem abierto");
	secretoRevelado = await cliente.request("VAULT_REVELAR_SECRETO", { resourceId: itemDetalleActual.id });
	return secretoRevelado;
}
botonRevelarPassword.addEventListener("click", async () => {
	if (botonRevelarPassword.getAttribute("aria-label") === "Ocultar contraseña") {
		detallePassword.textContent = "••••••••";
...
botonCopiarUsuario.addEventListener("click", () => {
	if (itemDetalleActual) copiarTexto(itemDetalleActual.usuario, botonCopiarUsuario);
});
...
botonCopiarComando.addEventListener("click", () => {
	copiarTexto(detalleComando.textContent ?? "", botonCopiarComando);
});
botonCopiarTotp.addEventListener("click", async () => {
	botonCopiarTotp.disabled = true;
	try {
		await copiarTexto((await obtenerSecretoRevelado()).totpSecret ?? "", botonCopiarTotp);
	} catch (error) {
...
formRecurso.addEventListener("submit", async (evento) => {
	...
	const datos = {
		...
		totpSecretBase32: formTotp.value.trim() || void 0
	};
	botonGuardarForm.disabled = true;
	botonGuardarForm.textContent = "Guardando…";
	try {
		if (modoFormulario === "crear") await cliente.request("VAULT_CREAR", { datos });
		else {
			if (!itemDetalleActual) throw new Error("no hay ningún ítem para editar");
			await cliente.request("VAULT_EDITAR", {
				item: itemDetalleActual,
				datos
			});
		}
		itemDetalleActual = null;
...
var generadorLargo = document.getElementById("generador-largo");
var generadorLargoValor = document.getElementById("generador-largo-valor");
...
var tabReciente = document.getElementById("tab-reciente");
